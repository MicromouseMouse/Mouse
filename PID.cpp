...
void PIDClass::PID(const PID_MODE &mode)   // stay in middle by led
{
	...
	case LED_MODE:
		...
		if (led->getLed(LEFT_DIAGONAL) > WALL_SIDE && led->getLed(RIGHT_DIAGONAL) > WALL_SIDE) // wall both side
		{
			error = led->getLed(LEFT_DIAGONAL) - led->getLed(RIGHT_DIAGONAL) - led->offsetLed;
		}
		else if (led->getLed(LEFT_DIAGONAL) > WALL_SIDE ) //wall left
		{
			error = led->getLed(LEFT_DIAGONAL) - led->leftThreshold;
		}
		else if (led->getLed(RIGHT_DIAGONAL) > WALL_SIDE )  //wall right
		{
			error = led->rightThreshold - led->getLed(RIGHT_DIAGONAL);
		}
		else PID(ENCODER_MODE);								// no wall will switch to encoder
